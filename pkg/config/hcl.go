...
func (s *HclEgressConfig) validate(serviceNames []string) error {
	if s.AllowAllServices && len(s.ConnectingServices) > 0 {
		return fmt.Errorf("cannot mix allow_all_services and connecting_services")
	}
...
type HclRoot struct {
	...
	Egresses  []*HclEgressConfig  `hcl:"egress,block"`
...
func (r *HclRoot) validate() error {
	...
	for _, egress := range r.Egresses {
		err := egress.validate(serviceNames)
		if err != nil {
			return fmt.Errorf("invalid service config %s/%s: %w", egress.Protocol, egress.Name, err)
		}
	}
